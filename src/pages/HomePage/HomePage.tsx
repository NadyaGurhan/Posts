...
export function HomePage(): JSX.Element {
  ...
  return (
    <div className={styles.container}>
      <h1>Список постов</h1>
      <p>
        Всего постов: {totalCount} | Страница: {currentPage} из {totalPages}
      </p>
            ...
            {posts.map((post) => (
              <article key={post.id} className={styles.postCard}>
                ...
                <div className={styles.postContent}>
                  <h2>
                    <Link to={`/post/${post.id}`} state={{ page: currentPage, limit }}>{post.title}</Link>
                  </h2>
                  <p>{post.body}</p>
                  <Link to={`/post/${post.id}`} state={{ page: currentPage, limit }} className={styles.button}>
                    Узнать подробнее →
                  </Link>
                </div>
              </article>
